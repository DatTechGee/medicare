@php
    $udCause = \App\Cause::find($id);
    $udScore = (int) ($udCause->fraud_score ?? 0);
    $udRisk = $udScore <= 20 ? 'low' : ($udScore <= 50 ? 'medium' : 'high');
    $udPct = $amount > 0 ? min(100, round((($raised ?? 0) / $amount) * 100)) : 0;
@endphp
<div class="mfc-card" style="animation: fadeInUp .5s ease both;">
    <div class="mfc-thumb">
        ...
        <div class="mfc-badges">
            @if(!empty($udCause->wallet_verified))
                <span class="mfc-badge mfc-badge--verified"><i class="fas fa-shield-halved"></i> {{__('Wallet Verified')}}</span>
            @endif
            @if($udRisk === 'low')
                <span class="mfc-badge mfc-badge--low"><i class="fas fa-circle-check"></i> {{__('Low Risk')}}</span>
            @elseif($udRisk === 'medium')
    ...
    <div class="mfc-body">
        <div class="mfc-progress-head">
            ...
            <span class="mfc-pct">{{$udPct}}%</span>
        </div>
        <div class="mfc-bar"><i style="width:{{$udPct}}%"></i></div>
        ...
        <div class="mfc-actions">
            @if($id)
                <a href="{{route('blockchain.donate.form', $id)}}" class="mfc-btn-donate"><i class="fas fa-heart"></i> {{$buttontext}}</a>
            @else
                <a href="{{route('frontend.donations.single',$slug)}}" class="mfc-btn-donate"><i class="fas fa-heart"></i> {{$buttontext}}</a>
            @endif
