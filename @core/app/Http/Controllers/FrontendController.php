...
class FrontendController extends Controller
{
    ...
    public function index()
    {
       ...
       $all_recent_donation = Cause::select('title','amount','cause_content','raised','image','slug','categories_id','excerpt','featured','id','reward')->where(['status' => 'publish'])
        ...
        if(in_array($home_page_variant,['07'])) {
            $donations_passing_homes = $all_recent_donation->take(9)->get();
            $events_passing_homes = $all_recent_events->take(3)->get();
            $blogs_passing_homes = $all_blog->take(3)->get();
        }
...
//        }
        if(!in_array($id,['01','02','03','04','05','06','07'])){
            abort(404);
        }
    ...
    public function ajax_login(Request $request)
    {
        ...
        if ($expected === null || $request->input('captcha_answer') === null || (int) $request->input('captcha_answer') !== (int) $expected) {
            return response()->json([
