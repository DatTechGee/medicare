...
class FrontendCausesController extends Controller
{
    ...
    {
        $query = Cause::where('status', 'publish');

        /* trust filters */
        if ($request->filled('wallet_verified')) {
            $query->where('wallet_verified', 1);
        }

        if ($request->filled('risk') && in_array($request->risk, ['low', 'medium', 'high'])) {
            $op = $request->risk === 'low' ? '<=' : '>';
            $bound = $request->risk === 'low' ? 20 : ($request->risk === 'medium' ? 50 : 50);
            $query->where('fraud_score', $op, $bound);
            if ($request->risk === 'high') {
                $query->orderByDesc('fraud_score');
            }
        }
        ...
        if ($request->filled('search')) {
            ...
            $query->where(function ($q) use ($s) {
                $q->where('title', 'like', $s)->orWhere('cause_content', 'like', $s);
            });
        }
        ...
        switch ($request->get('sort')) {
            case 'funded':
                $query->orderByRaw('CASE WHEN amount > 0 THEN (raised / amount) ELSE 0 END DESC');
                break;
            case 'urgency':
                $query->where('emmergency', 1)->orderBy('deadline', 'asc')->orderBy('id', 'desc');
                break;
            case 'trusted':
                $query->orderBy('fraud_score', 'asc')->orderByDesc('wallet_verified');
                break;
            default:
                $query->orderBy('id', 'desc');
        }

        $all_donations = $query->paginate(get_static_option('donor_page_post_items'))->withQueryString();

        return view(self::BASE_PATH . 'donation')->with([
            'all_donations' => $all_donations,
            'currentFilters' => $request->only(['wallet_verified', 'risk', 'sort', 'search']),
        ]);
    }
    ...
    /**
     * Legacy gateway-selection page now routes all donations through the blockchain demo flow.
     */
    public function redirect_to_blockchain_donation($id)
    {
        return redirect()->route('blockchain.donate.form', $id);
    }
