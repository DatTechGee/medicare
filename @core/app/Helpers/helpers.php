...
/* ---------------------------------------------------------------------------
 |  MediFund per-user generated blockchain accounts
 |  Every account (donor / patient / admin) gets a deterministic wallet on
 |  registration. Donors & admins are faucet-funded; patients start at zero
 |  and only RECEIVE after admin verification + approved disbursement.
 |--------------------------------------------------------------------------*/
if (!defined('MEDIFUND_ETH_USD_RATE')) {
    define('MEDIFUND_ETH_USD_RATE', 3450); /* demo rate used across the sim */
}

function medifund_usd_to_eth($usd)
{
    return round(((float) $usd) / MEDIFUND_ETH_USD_RATE, 6);
}

function generate_user_wallet_address($user)
{
    $seed = 'medifund-account:v1:user:' . $user->id . ':' . strtolower($user->email) . ':demo-network';
    return '0x' . substr(hash('sha256', $seed), 0, 40);
}

/**
 * Guarantees the user has a generated blockchain account.
 * Donors & admins receive a one-time demo faucet so they hold spendable funds.
 * Patients intentionally start unfunded — they only receive payouts after
 * wallet verification + disbursement approval by an admin.
 */
function ensure_user_wallet($user)
{
    ...
    if (empty($user->wallet_address)) {
        $faucetEth = in_array($user->role, ['donor', 'admin']) ? 10.0000 : 0.0000;
        $user->forceFill([
            'wallet_address' => generate_user_wallet_address($user),
            'wallet_connected_at' => now(),
            'demo_eth_balance' => max((float) ($user->demo_eth_balance ?? 0), $faucetEth),
        ])->save();

        \App\Helpers\AuditLogger::record('wallet_account_generated', 'User', $user->id, [
            'wallet_address' => $user->wallet_address,
            'role' => $user->role,
            'faucet_eth' => $faucetEth,
        ]);
    }
...
{
    $output = '<input type="hidden" name="selected_payment_gateway" value="blockchain_demo">';
    $output .= '<div style="background: linear-gradient(135deg, #0c1222, #0f172a); border: 1px solid rgba(13,148,136,0.3); border-radius: 14px; padding: 24px;">';
    $output .= '<div style="display: flex; align-items: center; gap: 12px; margin-bottom: 16px;">';
    $output .= '<div style="width: 44px; height: 44px; background: linear-gradient(135deg, #0d9488, #14b8a6); border-radius: 12px; display: flex; align-items: center; justify-content: center; color: #fff; font-size: 18px;"><i class="fas fa-link"></i></div>';
    $output .= '<div><div style="color: #f1f5f9; font-weight: 700; font-size: 16px;">Blockchain (Demo)</div>';
    $output .= '<div style="color: #14b8a6; font-size: 12px; font-weight: 500;">Secure & Transparent</div></div></div>';
    $output .= '<div style="background: rgba(13,148,136,0.08); border: 1px solid rgba(13,148,136,0.15); border-radius: 10px; padding: 14px; display: flex; align-items: center; gap: 10px;">';
    $output .= '<i class="fas fa-shield-alt" style="color: #14b8a6; font-size: 16px;"></i>';
    $output .= '<span style="color: #94a3b8; font-size: 13px; line-height: 1.5;">All donations are processed via blockchain demo. Funds are held in escrow and released upon milestone verification.</span>';
    $output .= '</div></div>';
    return $output;
}
...
{
    $output = '<option value="blockchain_demo" selected>Blockchain (Demo)</option>';
    return $output;
}
