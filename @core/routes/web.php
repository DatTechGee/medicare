...
Route::post('/connect-wallet', 'Api\BlockchainApiController@connectWallet')->name('api.blockchain.connect-wallet');
Route::get('/wallet-status', 'Api\BlockchainApiController@walletStatus')->name('api.blockchain.wallet-status');
Route::post('/disconnect-wallet', 'Api\BlockchainApiController@disconnectWallet')->name('api.blockchain.disconnect-wallet');
        Route::post('/auth/nonce', 'Auth\WalletAuthController@nonce')->name('api.blockchain.auth.nonce');
        Route::post('/auth/verify', 'Auth\WalletAuthController@verify')->name('api.blockchain.auth.verify');
        Route::post('/donate', 'Api\BlockchainApiController@processDonation')->name('api.blockchain.donate');
        Route::post('/verify-transaction', 'Api\BlockchainApiController@verifyTransaction')->name('api.blockchain.verify');
        Route::get('/campaign/{id}', 'Api\BlockchainApiController@campaignData')->name('api.blockchain.campaign-data');
        Route::get('/network-stats', 'Api\BlockchainApiController@networkStats')->name('api.blockchain.network-stats');
        Route::post('/release-escrow', 'Api\BlockchainApiController@releaseEscrow')->name('api.blockchain.release-escrow');
        Route::get('/config', 'Admin\AdminBlockchainController@config')->name('api.blockchain.config');
    });
...
----------------------------------*/
    Route::prefix('api')->group(function () {
        Route::get('/campaigns', 'Api\PublicApiController@campaigns')->name('api.public.campaigns');
        Route::get('/campaigns/{id}', 'Api\PublicApiController@campaign')->where('id', '[0-9]+')->name('api.public.campaign');
        Route::get('/tx/{hash}', 'Api\PublicApiController@transaction')->where('hash', '[a-fA-F0-9x]+')->name('api.public.tx');
...
Route::post('/reject/{id}', 'PatientWalletController@reject')->name('admin.patient.wallet.reject');
    ...
    /*----------------------------------------------------------------------------------------------------------------------------
     | DOCUMENT INTEGRITY (ADMIN) — SHA-256 tamper-evidence for medical documents
     |----------------------------------------------------------------------------------------------------------------------------*/
    ...
    /*----------------------------------------------------------------------------------------------------------------------------
     | ESCROW SAFETY ACTIONS (ADMIN) — refund / freeze / blacklist with audit trail
     |----------------------------------------------------------------------------------------------------------------------------*/
